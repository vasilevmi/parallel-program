...
public:
    ...
    Data(int length) {
        len = length;       
        arr = new ctype[len];    
        // Для использования с OpenACC, копируем указатель и данные массива в память устройства
    ...
    // Деструктор
    ~Data() {
        // Для использования с OpenACC, удаляем массив из памяти устройства
...
double linearInterpolation(double x, double x1, double y1, double x2, double y2) {
    // делаем значение клетки матрицы используя формулу линейной интерполяции
