...
#pragma acc host_data use_device(curmatrix,prevmatrix)
                {
                    stat = cublasDcopy(handle, N * N, curmatrix, 1, prevmatrix, 1);// гарантирует, что последующие итерации  будут использовать правильные значения, включая краевые значения, которые могут зависеть от значений вокруг них. Используем значения из предыдущей итерации для вычисления новых значений.
